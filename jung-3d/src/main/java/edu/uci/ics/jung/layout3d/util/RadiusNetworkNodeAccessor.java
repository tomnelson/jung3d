...
public class RadiusNetworkNodeAccessor<N> implements NetworkNodeAccessor<N> {
  ...
  public RadiusNetworkNodeAccessor(Graph<N> graph, LayoutModel<N> layoutModel) {
    this(graph, layoutModel, Math.sqrt(Double.MAX_VALUE - 1000));
  }
   ...
   */
  public RadiusNetworkNodeAccessor(Graph<N> graph, LayoutModel<N> layoutModel, double maxDistance) {
    this.graph = graph;
   ...
   */
  @Override
  public N getNode(double x, double y, double z) {
    ...
    //    log.info("winner took {}", System.currentTimeMillis() - time);
    return closest;
  }
}
